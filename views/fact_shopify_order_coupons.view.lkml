view: fact_shopify_order_coupons {
  sql_table_name: fact_shopify_order_coupons ;;
  view_label: "Shopify Order Coupons"
...
####### END COMPARISON DIMENSIONS/FILTERS #######
  ...
  dimension: primary_key {
    type: string
    sql:CONCAT(CONCAT(CAST(${order_id} as VARCHAR), CAST(${glew_account_id} as VARCHAR)),  CAST(${coupon_code} as VARCHAR)) ;;
    primary_key: yes
    hidden: yes
  }
  ...
  measure: number_of_order_coupons {
    type: count_distinct
    sql: ${order_id} ;;
    group_label: "Standard Measures"
    description: "Distinct Count of Coupon Code Order ID"
  }
